        ...
        if key and value:
            ...
            if key == "학교":
                if args[3].isdigit() and args[4].isdigit():
                    ...
                    if user_data:
                        await db.update_school(ctx.author.id, args[0], args[1], grade, class_nm, args[2])
                    else:
                        await db.create_user_data(ctx.author.id, args[0], args[1], grade, class_nm, args[2])
                    return await ctx.send(embed=discord.Embed(title="학교 정보가 설정되었습니다."))
                else:
                    return await ctx.send(embed=discord.Embed(title="잘못된값을 주셨습니다. 처음부터 다시 시도해주세요."))
            elif key == "공개":
                user_data = await db.get_user_data(ctx.author.id)
                if user_data:
                    if value in [
